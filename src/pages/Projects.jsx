...
export default function ProjectManagement() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'hosting_amount') {
      const numValue = value === '' ? null : Number(value);
      setForm(prev => ({ ...prev, [name]: numValue }));
    }

    else if (['beta_release_date', 'live_date', 'hosting_start_date'].includes(name)) {
  ...
  const cleanPayload = (payload) => {
    ...
    Object.keys(cleaned).forEach((key) => {
      ...
      if (key === 'id') delete cleaned[key];
        ...
        !['project_code', 'project_name', 'production_url', 'production_server', 'currency', 'status', 'beta_url', 'beta_server'].includes(key)
      ) {
        delete cleaned[key];
      } else if (dateFields.includes(key) && isNaN(Date.parse(value))) {
        delete cleaned[key];
      }
    });

    if (cleaned.hosting_amount === null) {
      cleaned.hosting_amount = 0;
    }
    return cleaned;
  };
        ...
        <section className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex gap-2">
            <input
              ...
              className="px-4 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
            />
            <select
              className="px-4 py-2 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer transition"
              value={statusFilter}
              onChange={async (e) => {
                ...
                {projects
                  ...
                  .map((p, idx) => (
                    <tr key={p.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-blue-50'}>
                      ...
                      <td className="p-2 border">{p.company_name || '-'}</td>
